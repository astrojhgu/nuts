...
use crate::{nuts::{AdaptStrategy, Hamiltonian, NutsChain, State}, Chain};
    ...
    R: Rng
{
    ...
    for i in (1..nbeta).rev() {
        ...
        for (j2, &j1) in jvec.iter().enumerate() {
            ...
            let lp2 = -chains[(i - 1) * nchains_per_beta + j2].current.potential_energy();
            ...
            if r < ep {
                ...
                chains[(i-1)*nchains_per_beta+j2].current.write_position(&mut p1);
                chains[i*nchains_per_beta+j1].current.write_position(&mut p2);

                chains[(i-1)*nchains_per_beta+j2].set_position(& p2).unwrap();
